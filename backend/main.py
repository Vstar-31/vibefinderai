...
async def analyze_vibe(request: VibeRequest, token: str = Depends(oauth2_scheme)):
    ...
    if vibe_data.get("confidence", 0.0) < 0.25 and not detected_artist and not request.override_genre:
        ...
        # v8 FIX: Multi-stage retry when direct search returns empty pool
        if not raw_pool:
            logger.warning(f"Direct Search pool empty for '{request.text}'. Trying keyword extraction retry...")
            # Stage 1: Strip stopwords and retry with keywords only
            STOP_WORDS = {"the","a","an","of","for","to","at","in","on","is","it","type","vibe","music",
                          "song","playlist","chill","feel","feels","feeling","style","kind","sort","genre"}
            keywords = [w for w in re.findall(r"[a-z']+", request.text.lower()) if w not in STOP_WORDS and len(w) > 2]
            if keywords:
                retry_query = " ".join(keywords[:4])
                logger.info(f"Stage 1 retry query: '{retry_query}'")
                raw_pool = await fetch_lastfm_track_search(retry_query, limit=100)
                logger.info(f"Stage 1 retry returned {len(raw_pool)} tracks.")

        if not raw_pool:
            # Stage 2: Try longest token as artist search
            words_sorted = sorted([w for w in re.findall(r"[a-z']+", request.text.lower()) if len(w) > 4], key=len, reverse=True)
            if words_sorted:
                artist_guess = words_sorted[0]
                logger.info(f"Stage 2 artist guess: '{artist_guess}'")
                raw_pool = await fetch_lastfm_artist_tracks(artist_guess)
                logger.info(f"Stage 2 artist search returned {len(raw_pool)} tracks.")

        if not raw_pool:
            # Stage 3: Hard fallback — use the dreamy/chill genre pool as a safe default
            logger.warning("Stage 3 hard fallback: using dreamy genre pool.")
            fallback_genres = ["dream pop", "indie pop", "chillwave"]
            for fg in fallback_genres:
                raw_pool = await fetch_lastfm_tracks(fg, language=request.language, limit=50)
                if raw_pool:
                    logger.info(f"Stage 3 fallback genre '{fg}' returned {len(raw_pool)} tracks.")
                    break
