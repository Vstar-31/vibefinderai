...
def _build_logger(log_file: Path) -> logging.Logger:
    logger = logging.getLogger("gemini_log_grader")
...
async def _grade_rows(
    ...
    timeout = aiohttp.ClientTimeout(total=None)  # per-call timeout managed via asyncio.wait_for
    total = len(rows)
    done_count = 0
    timed_out_count = 0
    ...
    try:
        async with aiohttp.ClientSession(connector=connector, timeout=timeout) as session:
            ...
            async def run_one(idx: int, row: Dict[str, Any]) -> Dict[str, Any]:
                # Skip already-graded rows (resume support)
                ...
                async with sem:
                    ...
                    # Stream write — safe to interrupt
...
async def main() -> None:
    ...
    if already_graded:
        # If resuming, append to the existing file instead of creating a new one
...
async def _call_gemini(session: aiohttp.ClientSession, row: Dict[str, Any], retries: int = 4) -> Dict[str, Any]:
    system_prompt, user_prompt = _build_gemini_prompt(row)
    payload = {
        "contents": [{"parts": [{"text": user_prompt}]}],
        "systemInstruction": {"parts": [{"text": system_prompt}]},
...
async def _grade_rows(rows: List[Dict[str, Any]], concurrency: int, logger: logging.Logger) -> List[Dict[str, Any]]:
    sem = asyncio.Semaphore(concurrency)
    connector = aiohttp.TCPConnector(limit=max(16, concurrency * 3))
    ...
    async with aiohttp.ClientSession(connector=connector, timeout=timeout) as session:
        async def run_one(idx: int, row: Dict[str, Any]) -> Dict[str, Any]:
            async with sem:
                grade = await _call_gemini(session, row)
                output = dict(row)
                output["gemini_relevance"] = grade
                output["gemini_graded_at"] = datetime.now().isoformat()
                output["gemini_graded_index"] = idx
...
async def main() -> None:
    ...
    logger.info(f"Input file: {input_path}")
    logger.info(f"Rows to grade: {len(rows)}")
    logger.info(f"Concurrency: {args.concurrency}")
    logger.info(f"Live log file: {live_log_path}")

    graded_rows = await _grade_rows(rows, max(1, args.concurrency), logger)
    summary = _summarize(graded_rows)
    summary["input_file"] = str(input_path)
    summary["output_file"] = str(output_path)
    summary["live_log_file"] = str(live_log_path)

    _write_jsonl(output_path, graded_rows)
    _write_json(summary_path, summary)
