...
async def youtube_create_playlist(body: YTPlaylistRequest, authorization: str = Query(...)):
    ...
    async def _yt_req(method: str, url: str, **kwargs):
        nonlocal yt_token
    ...
    # 2. Search + add each track
    for t in body.tracks[:50]:
        q = f"{t.get('title', '')} {t.get('artist', '')} official"
        search_result = await youtube_search(q=q)
        if not search_result.get("found"):
            continue
        video_id = search_result["video_id"]
        add_resp = await _yt_req(
            "post",
    ...
    logger.info(f"[Services] YouTube playlist '{body.name}' created ({added} tracks) for user {user_id[:8]}...")
